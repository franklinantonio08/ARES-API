...
class OperativoController extends Controller{
    ...
    public function EditarOperacion(Request $request, $id){
        ...
        $infractor = Infractor::where('id', $infractorId)
            ->where('usuarioId', $userId)
            ->first();

        if (!empty($infractor)) {

            $tipoEvento = $infractor->tipo_evento; 

            if($tipoEvento === "Operativo"){

                $evento = Infractoresoperativo::where('infractorId', $infractorId)
                    ->where('usuarioId', $userId)
                    ->first();

            }else{

                $evento = Infractoresincidencia::where('infractorId', $infractorId)
                    ->where('usuarioId', $userId)
                    ->first();
